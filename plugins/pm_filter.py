...
async def next_page(bot, query):
    ...
    if settings['button']:
                    ...
                    btn = [
                        [
                            InlineKeyboardButton(
                                ...
                                url=await get_shortlink(f"https://telegram.me/{temp.Fz_IMDB_FilesBot}?start=files_{file.file_id}")
                            ),
                        ]
                        for file in files
                    ]
        ...
        if URL_MODE is True:
            if query.from_user.id in ADMINS:
                ...
                btn = [
                    [
                        InlineKeyboardButton(text=f"{file.file_name}",url=await get_shortlink(f"https://telegram.me/{temp.Fz_IMDB_FilesBot}?start=files_{file.file_id}")),
                        InlineKeyboardButton(text=f"[{get_size(file.file_size)}]", url=await get_shortlink(f"https://telegram.me/{temp.Fz_IMDB_FilesBot}?start=files_{file.file_id}")),
                    ]
                    for file in files
                ]
        else:
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/howtoopenshortnerlinks/14')
        ] 
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Watch Tutorial', url='https://t.me/howtoopenshortnerlinks/14')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        try:
            btn = [[
                ...
                InlineKeyboardButton(text=f"🐞 REPORT BUG 🐞", url=f"https://telegram.me/stylishboyfaiz")
            ],[
                InlineKeyboardButton(text=f"⚡️ Learn Bot Making Dm 🦋", url=f"https://t.me/stylishboyfaiz")
        ...
        try:
            btn = [[
                ...
                InlineKeyboardButton(text=f"🐞 REPORT BUG 🐞", url=f"https://t.me/stylishboyfaiz")
            ],[
                InlineKeyboardButton(text=f"⚡️ Learn Bot Making Dm 🦋", url=f"https://t.me/stylishboyfaiz")
            ]]
            btn_lzdv = [
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ to ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/howtoopenshortnerlinks/14'),
        ] 
    )
    if offset != "":
